...
function move_data() {
	...
	pushd models/metrics
		echo "moving" "${path}/sys_metrics_${suffix}.csv", "${path}/stat_metrics_${suffix}.csv"
...
function run_fedavg() {
	...
	pushd models/
		echo python main.py -dataset 'femnist' -model 'cnn' --num-rounds ${num_rounds} --clients-per-round ${clients_per_round} --num-epochs ${num_epochs} -lr ${fedavg_lr}
...
function run_minibatch() {
	...
	pushd models/
		echo python main.py -dataset 'femnist' -model 'cnn' --minibatch ${minibatch_percentage} --num-rounds ${num_rounds} --clients-per-round ${clients_per_round} -lr ${minibatch_lr}
