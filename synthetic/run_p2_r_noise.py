...
def main():
    ...
    cfg = {
        ...
        "n" : [100],
        "d" : [1000],
        "noise_scale" : [0.0, 0.1, 0.2, 0.4],
        "r" : [0.1, 0.25, 0.5, 0.75, 1.0],
        "data_seed":range(40),
        "train_seed":range(10),
        'lr' : [1.0, 0.1, 0.01], # originally 1.0
    }
...
class MyProcessRunner(ProcessRunner):
    def summarize(self, force = False):
        ...
        print(f"noise_scale {cfg2['noise_scale'][0]}")

        for cfg in cfgs2:
            key1 = cfg.keys()
            key1v = cfg.values()

            THRE = THRE0 * cfg["noise_scale"] + 0.001
            ...
            plot_data[ (cfg['noise_scale'], cfg['r']) ] = success_rate
